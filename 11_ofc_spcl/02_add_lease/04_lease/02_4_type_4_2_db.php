...
$ad_id = $_SESSION['ofc_spcl'];

//รับ Post ข้อมูล
$le_id = $_POST['le_id'];
$area_id = $_POST['area_room'];
...
//ข้อมูลสถานะภาพ
$le_4_role = $_POST['le_4_role'];
$le_4_fact = $_POST['le_4_fact'];
...
{ //ผู้ให้เช่า
    ...
    //ผู้จัดการหอพัก
    $ofc_2 = "SELECT * FROM officer WHERE ofc_type = 5";
...
if ($result_q2 && $result_q3 && $result_q6 && $result_q7 && $result_q8) {
    ...
    //แจ้งเตือนเจ้าหน้าที่การเงินทุกคน
    {
        ...
        while ($r1 = mysqli_fetch_assoc($result1)) { // วนลูปเจ้าหน้าที่การเงินทุกคน
            ...
            $detail = "สัญญาเพื่อที่พักอาศัย เลขที่สัญญาเช่า $le_no ต้องชำระค่าเช่าล่วงหน้า และเงินประกัน";
    ...
    echo "<script type='text/javascript'>";
    echo "alert('เพิ่มข้อมูลสัญญาเช่า: ประเภทสัญญาเพื่อที่พักอาศัย สำเร็จ \\nรอเจ้าหน้าที่การเงิน เพิ่มข้อมูลค่าเช่าล่วงหน้า และเงินประกัน'); ";
    echo "window.location='../02_1_do_lease.php';";
    echo "</script>";
} else {
    echo "<script type='text/javascript'>";
    echo "alert('มีความผิดพลาดll');";
    echo "window.location='../02_1_do_lease.php';";
    echo "</script>";
}
