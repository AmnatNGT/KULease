<?php {
    ...
    // ดึงข้อมูลพื้นที่เช่า ที่เป็นของประเภทสัญญาที่ 3
    $sql = "SELECT * FROM area WHERE area_type = 3 AND area_status_show = 1 ORDER BY area_name ASC"; //ดึงข้อมูลทั้งหมดออกมาก และเรียงตามพยัญชนะชื่อ
    $result = mysqli_query($conn, $sql); //ข้อมูลที่ดึงมาไว้ในตัวแปร result
    $count = mysqli_num_rows($result); //นับจำนวณแถวในฐานข้อมูล
...
<body>
    ...
    <main class="data ">
        <div class="wrapper_2">
            <div class="title">
                สถานะพื้นที่เช่า ประเภทสัญญาเพื่องานวิจัย / การเรียนการสอน
            </div>
                    ...
                    <table id="customers">
                        ...
                        <tbody>
                            <?php while ($row = mysqli_fetch_assoc($result)) { ?>
                                ...
                                <?php if ($row["area_status"] == '0') { ?>
                                        <td>
                                            <!-- กดแก้ไข ไปหน้า 05_3_4_area_edit.php ส่ง id พื้นที่ -->
                                            <a href="05_3_4_area_edit.php?idsub=<?php echo $row["area_id"] ?>" class="btn btn-warning">แก้ไข</a>
                                            <!-- กดลบ ไปหน้า 05_3_3_delete_area_db.php ส่ง id พื้นที่ -->
                                            <a href="05_3_3_delete_area_db.php?idsub=<?php echo $row["area_id"] ?>" class="btn btn-danger" onclick="return confirm('ยืนยันการลบ')">ลบ</a>
                                        </td>
                                    <?php } 
         ...
         <!--เพิ่มพื้นที่เช่า-->
        <!-- ส่งข้อมูลไปหน้า 05_3_2_area_add_db.php -->
        <form action="05_3_2_area_add_db.php" method="POST">

            <div class="modal fade" id="showForm1">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <div>เพิ่มพื้นที่เช่า ประเภทสัญญาเพื่องานวิจัย / การเรียนการสอน</div>
                        ...
                        <div class="modal-body">
                            ...
                            <div class="wrapper_md">
                                ...
                                <div class="form">
                                    ...
                                    <div class="inputfield">
                                        <label>ขนาดพื้นที่เช่า (ไร่) <strong style="color:red;">*</strong></label>
                                        <input type="text" class="input" placeholder="ขนาดพื้นที่เช่า (ไร่)" name="area_size" required>
                                    </div>
