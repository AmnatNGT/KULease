<?php {
    ...
    // ดึงข้อมูลพื้นที่เช่า ที่เป็นของประเภทสัญญาที่ 4
    $sql = "SELECT * FROM area WHERE area_type = 4 AND area_status_show = 1 ORDER BY area_name ASC"; //ดึงข้อมูลทั้งหมดออกมาก และเรียงตามพยัญชนะชื่อ
...
<body>
    ...
    <main class="data ">
        <div class="wrapper_2">
            <div class="title">
                สถานะพื้นที่เช่า ประเภทสัญญาเพื่อที่พักอาศัย
            </div>
                        ...
                        <tbody>
                            ...
                            <?php while ($row = mysqli_fetch_assoc($result)) { ?>
                                <tr>
                                    ...
                                    <td> <?php echo $row["area_room_no"] ?> </td>
                                ...
                                <?php if ($row["area_status"] == '0') { ?>
                                    ...
                                    <?php if ($row["area_status"] == '0') { ?>
                                        <td>
                                            <!-- กดแก้ไข ไปหน้า 05_4_4_area_edit.php ส่ง id พื้นที่ -->
                                            <a href="05_4_4_area_edit.php?idsub=<?php echo $row["area_id"] ?>" class="btn btn-warning">แก้ไข</a>
                                             <!-- กดลบ ไปหน้า 05_4_3_delete_area_db.php ส่ง id พื้นที่ -->
                                             <a href="05_4_3_delete_area_db.php?idsub=<?php echo $row["area_id"] ?>" class="btn btn-danger" onclick="return confirm('ยืนยันการลบ')">ลบ</a>
                                        </td>
                                    <?php } else 
        ...
        <!--เพิ่มพื้นที่เช่า-->
        <!-- ส่งข้อมูลไปหน้า 05_4_2_area_add_db.php -->
        <form action="05_4_2_area_add_db.php" method="POST">

            <div class="modal fade" id="showForm1">
                <div class="modal-dialog modal-lg">
                    <div class="modal-content">
                        <div class="modal-header">
                            <div>เพิ่มพื้นที่เช่า ประเภทสัญญาเพื่อที่พักอาศัย</div>
