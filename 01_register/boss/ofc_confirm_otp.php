...
if (isset($_POST['submit'])) {
    ...
    if ($sql_otp === $otp) {
        ...
        $ch_d = $row['ls_otp_date'];
        $ch_t = $row['ls_otp_time'];
        ...
        //ถ้า วันและเวลาปัจจุบัน < วันและเวลาที่ otp หมดอายุ
            ...
            //ให้ UPDATE ls_status_otp =1
            $sql_otp_2 = "UPDATE lessor SET ls_status_otp = 1
               WHERE ls_id = $ofc_id";
                ...
                $detail =
                    '<!DOCTYPE html>
                        ...
                        <div class="wrapper">
                            ...
                            <a href="https://cskps.flas.kps.ku.ac.th/kulease2/user_verify_email.php?boss_id=' . $ofc_id . ' ">
...
//ถ้าได้รับ $_POST['new_otp'] ให้ทำงานที่นี้
    ...
    if ($count_otp < 3) {
        ...
        //update new otp และ วันเวลาที่  otp หมดอายุ ไปเก็บไว้ที่ ตาราง lessor
        $upd_otp = "UPDATE lessor SET ls_otp = '$otp',
                                   ls_otp_date = '$date_otp',
                                   ls_otp_time = '$time_otp',
                                   count_otp = '$count_otp'
                               WHERE ls_id = $ofc_id";
        $result_upd_otp = mysqli_query($conn, $upd_otp);

        //เตรียม email และ เบอร์ ของผู้ให้เช่า
...
<body>
    ...
    <div class="wrapper">
        ...
        <form action="" method="POST">
            <div class="form">
                ...
                <div class="inputfield">
                    ...
                    <input type="text" class="input" placeholder="หมายเลข OTP" name="ls_otp" maxlength="5" required>
                </div>
                <!-- กรอกเลข OTP -->
                <div class="inputfield">
        ...
        <div class="inputfield">
            ...
            <input type="text" class="input" value="<?php echo date('d / m / Y ', strtotime($row['ls_otp_date'])); ?>" readonly>
        </div>

        <div class="inputfield">
            <label>เวลา</label>
            <input type="text" class="input" value="<?php echo $row['ls_otp_time']; ?> น." readonly>
        </div>
