...
if (isset($_POST['submit'])) {
    ...
    if ($sql_otp === $otp) {
        ...
        $ch_d = $row['otp_date_s2'];
        $ch_t = $row['otp_time_s2'];
        ...
        //ถ้า วันและเวลาปัจจุบัน < วันและเวลาที่ otp หมดอายุ
            ...
            //update status_lease ให้ไปสถานะ ผู้จัดการหอพักลงนาม
            $q7 = "UPDATE status_lease SET st_s2='1', d_st_s2='$d_now', t_st_s2='$t_now', st_s3='0' WHERE le_id = $le_id ";
            $result_q7 = mysqli_query($conn, $q7);

            //update ลายเซน 
            $q8 = "UPDATE lease SET ofc1_sign_pic = '$fullname' WHERE le_id = $le_id ";
...
//ถ้าได้รับ $_POST['new_otp'] ให้ทำงานที่นี้
    ...
    if ($count_otp < 3) {
        ...
        $q7 = "UPDATE status_lease SET otp_s2='$otp',
                                otp_date_s2='$date_otp', 
                                otp_time_s2='$time_otp',
                                count_otp_s2 ='$count_otp'  
                                WHERE le_id = $le_id ";
        $result_q7 = mysqli_query($conn, $q7);

        // id เจ้าหน้าที่พิเศษจาก session
        $ofc_id = $_SESSION['ofc_dmt'];

        //ดึงข้อมูล ผู้จัดการหอพัก
...
<body>
    ...
    <div class="bar_name">
        ...
        <span class="name"><?php echo $_SESSION['ofc_dmt_name']; ?></span> <br>
        <span class="name"><strong>( ผู้จัดการหอพัก )</strong></span>
    </div>
    ...
    <div class="wrapper">
        ...
        <div class="inputfield">
            ...
            <input type="text" class="input" value="<?php echo date('d / m / Y ', strtotime($row['otp_date_s2'])); ?>" readonly>
        </div>

        <div class="inputfield">
            <label>เวลา</label>
            <input type="text" class="input" value="<?php echo $row['otp_time_s2']; ?> น." readonly>
        </div>
