<?php {
    ...
    // แสดงข้อมูลผู้เช่า ที่ tn_status_use = '0' and tn_verify= '1'
    $sql_tn = "SELECT * FROM tenant 
                        WHERE tn_status_use = '0' 
                        and tn_verify= '1' "; 
...
<body>
    ...
    <main class="data ">
        <div class="wrapper_2">
            <div class="title">
                รายชื่อผู้เช่า ที่ประสงค์ลงทะเบียนเข้าใช้งานระบบ
            </div>
                        ...
                        <tbody>
                            ...
                            <?php while ($row_tn = mysqli_fetch_assoc($result_tn)) { ?>
                                <tr>
                                    ...
                                    <td>
                                        <a STYLE="color: blue;" href="08_4_information_tn_regis.php?idsub=<?php echo $row_tn["tn_id"] ?>"> <?php echo  $row_tn["tn_p_name"] . " " . $row_tn["tn_f_name"] . " " . $row_tn["tn_l_name"]; ?> </a>
                                    </td>
                                    <td><?php echo date('d / m / Y ', strtotime($row_tn["tn_date_regis"])); ?> </td>
                                    <td><?php echo $row_tn["tn_time_regis"]; ?> </td>
                                    <td>ผู้เช่า</td>

                                    <td>
                                        <!-- ถ้ากดอนุมัติ ไปหน้า  08_3_admin_check_tn_register_confirm_db.php ส่ง id ผู้เช่าไปด้วย-->
                                        <a STYLE="color: white;" href="08_3_admin_check_tn_register_confirm_db.php?idsub=<?php echo $row_tn["tn_id"] ?>" class="btn_yes" onclick="return confirm('ยืนยันการอนุมัติ')">อนุมัติ</a>
                                        <!-- ถ้าไม่กดอนุมัติ ไปหน้า  08_2_admin_check_tn_register_delete_db.php ส่ง id ผู้เช่าไปด้วย-->
                                        <a STYLE="color: white;" href="08_2_admin_check_tn_register_delete_db.php?idsub=<?php echo $row_tn["tn_id"] ?>" class="btn_no" onclick="return confirm('ยืนยันการไม่อนุมัติ')">ไม่อนุมัติ</a>
                                    </td>
