...
if (isset($_POST['submit'])) {
    ...
    if ($sql_otp === $otp) {
        ...
        $ch_d = $row['otp_date_tn1'];
        $ch_t = $row['otp_time_tn1'];
        ...
        //ถ้า วันและเวลาปัจจุบัน < วันและเวลาที่ otp หมดอายุ
            ...
            //update ชื่อลายเซนผู้เช่า
            $q8 = "UPDATE lease SET tn_sign_pic = '$fullname' WHERE le_id = $le_id ";
            $result_q8 = mysqli_query($conn, $q8);

            if ($result_q8) {

                echo "<script type='text/javascript'>";
                echo "alert('ยืนยัน OTP สำเร็จ ต่อไปให้พยานของผู้เช่าลงนาม');";
                // เมื่อเสร็จไปหน้า 09_1_chk_data.php
                echo "window.location='09_1_chk_data.php';";
                echo "</script>";
            } else {
                echo "<script type='text/javascript'>";
                echo "alert('ผิดพลาด');";
...
//ถ้าได้รับ $_POST['new_otp'] ให้ทำงานที่นี้
    ...
    if ($count_otp < 3) {
        ...
        $q7 = "UPDATE status_lease SET otp_t1='$otp',
                                otp_date_tn1='$date_otp', 
                                otp_time_tn1='$time_otp',
                                count_otp_tn1 ='$count_otp'  
                                WHERE le_id = $le_id ";
        ...
        if ($result_q7) {
            ...
            //ให้กลับมาหน้า 05_confirm_otp.php page (มาหน้าเดิม)และ ส่งค่า $id แบบ get ไปด้วย
            echo "window.location='05_confirm_otp.php'; ";
            echo "</script>";
        } else {
            echo "<script type='text/javascript'>";
...
<body>
    ...
    <div class="wrapper">
        ...
        <div class="inputfield">
            ...
            <input type="text" class="input" value="<?php echo date('d / m / Y ', strtotime($row['otp_date_tn1'])); ?>" readonly>
        </div>

        <div class="inputfield">
            <label>เวลา</label>
            <input type="text" class="input" value="<?php echo $row['otp_time_tn1']; ?> น." readonly>
        </div>
