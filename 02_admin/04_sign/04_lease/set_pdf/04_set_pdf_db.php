...
//แจ้งเตือนผู้จัดการหอพัก ลงนาม
{
    ...
    //หาข้อมูลผู้จัดการหอพัก
    $sql2 = "SELECT * FROM officer WHERE ofc_type = 5";
    $result2 = mysqli_query($conn, $sql2);
    $row2 = mysqli_fetch_assoc($result2);

    //send otp
    // email ผู้จัดการหอพัก
    $tn_email = $row2['ofc_email'];
    // phone ผู้จัดการหอพัก
    $tn_tel = $row2['ofc_tel'];
    $detail = "สัญญาเพื่อที่พักอาศัย เลขที่สัญญาเช่า : $leno ผู้เช่าลงนามเสร็จแล้ว ต่อไปอยู่ในสถานะเจ้าหน้าที่ลงนาม โปรดตรวจสอบ";

    //send otp
    // send email ผู้จัดการหอพัก
