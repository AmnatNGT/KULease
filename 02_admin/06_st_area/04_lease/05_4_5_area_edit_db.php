...
if (isset($_POST['e1'])) {
    ...
    if ($result) {
        ...
        // เมื่อเสร็จแล้วไปที่หน้า 05_4_4_area_edit.php ส่ง id พื้นที่เช่าไปด้วย
        echo "window.location='05_4_4_area_edit.php?idsub=$area_id';";
        echo "</script>";
    } else {
        echo "<script type='text/javascript'>";
...
//ห้อง e3
    ...
    // รับข้อมูล
    $area_id = $_POST['area_id'];
    $data = $_POST['area_room_no'];

    // update พื้นที่
    $sql = "UPDATE area set area_room_no = '$data' WHERE area_id = $area_id ";
    $result = mysqli_query($conn, $sql);
