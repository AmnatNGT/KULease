<?php {
    ...
    //ค่าเช่ารายเดือน
    $sql_mn_month  = "SELECT * FROM money WHERE le_id = $le_id AND mn_type = 4 ";
    $result_mn_month  = mysqli_query($conn, $sql_mn_month);
    $row_mn_month = mysqli_fetch_assoc($result_mn_month);
    //ข้อมูลค่าเช่ารายเดือน
    {
        $month_cost = thainumDigit($row_mn_month['mn_cost']);
        $year_cost = thainumDigit($row_mn_month['mn_cost'] * 12);
    ...
    {
        $ad_cost = thainumDigit($row_mn_ad['mn_cost']);
        $ad_no = $row_mn_ad['mn_no']; //เลขที่ใบเสร็จ
        $ad_volume = $row_mn_ad['mn_volume'];
        $ad_date_pay = date('d', strtotime($row_mn_ad["mn_date_pay"]));
        $ad_month_pay = cal_month(date('m', strtotime($row_mn_ad["mn_date_pay"])));

        $ad_year_pay =  date('Y', strtotime($row_mn_dp["mn_date_pay"]));
...
//เอกสารแนบท้าย
{
    ...
    //Set ตำแหน่งข้อมูล
    {
        ...
        $html_last .= '<div style="position:absolute;top:877;left:385px;width:365px; text-align:center;"> อธิการบดีมหาวิทยาลัยเกษตรศาสตร์ </div>';
    }
...
if ($row_mn_dp['mn_volume'] != null) {
    ...
    //Set ตำแหน่งข้อมูล
    {
        ...
        $html .= '<div style="position:absolute; top:176px; left:630px;width:60px; text-align:center;"> ' . $date_today . '</div>';
        $html .= '<div style="position:absolute;top:208;left:145px;width:115px; text-align:center;"> ' . $month_today . ' </div>';
        $html .= '<div style="position:absolute;top:208;left:280px;width:80px; text-align:center;"> ' . $year_today . ' </div>';
        $html .= '<div style="position:absolute;top:240;left:140px;"> ' . $boss_name . ' </div>';
        $html .= '<div style="position:absolute;top:272;left:165px;"> ' . $boss_role . ' </div>';
        $html .= '<div style="position:absolute;top:304;left:365px;"> ' . $tn_name . ' </div>';
        $html .= '<div style="position:absolute;top:337;left:135px;width:65px; text-align:center;"> ' . $tn_age . ' </div>';
        $html .= '<div style="position:absolute;top:337;left:260px;width:80px; text-align:center;"> ' . $tn_ethnicity . ' </div>';
        $html .= '<div style="position:absolute;top:337;left:385px;width:75px; text-align:center;"> ' . $tn_nationality . ' </div>';
        $html .= '<div style="position:absolute;top:337;left:540px;width:65px; text-align:center;"> ' . $tn_house_no . ' </div>';
        $html .= '<div style="position:absolute;top:337;left:640px;width:75px; text-align:center;"> ' . $tn_moo . ' </div>';

        $html .= '<div style="position:absolute;top:369;left:150px;width:120px; text-align:center;"> ' . $tn_alley . ' </div>';
        $html .= '<div style="position:absolute;top:369;left:280px;width:90px; text-align:center;"> ' . $tn_road . ' </div>';
        $html .= '<div style="position:absolute;top:369;left:450px;width:90px; text-align:center;"> ' . $tn_canton . ' </div>';
        $html .= '<div style="position:absolute;top:369;left:610px;width:100px; text-align:center;">' . $tn_district . ' </div>';
        $html .= '<div style="position:absolute;top:402;left:150px;width:100px; text-align:center;"> ' . $tn_province . ' </div>';

        $html .= '<div style="position:absolute;top:498;left:405px;">' . $area_name . ' </div>';
        $html .= '<div style="position:absolute;top:603;left:220px;width:90px; text-align:center;"> ' . $area_size . '</div>';
        $html .= '<div style="position:absolute;top:603;left:380px;">' . $le_purpose . '</div>';
        $html .= '<div style="position:absolute;top:629;left:180px;width:60px; text-align:center;">  ' . $le_duration . '</div>';
        $html .= '<div style="position:absolute;top:629;left:333px;width:45px; text-align:center;"> ' . $le_start_date . '</div>';
        $html .= '<div style="position:absolute;top:629;left:400px;width:110px; text-align:center;">' . $le_start_month . '</div>';
        $html .= '<div style="position:absolute;top:629;left:555px;width:70px; text-align:center;"> ' . $le_start_year . '</div>';
        $html .= '<div style="position:absolute;top:629;left:660px;width:50px; text-align:center;"> ' . $le_end_date . '</div>';
        $html .= '<div style="position:absolute;top:656;left:140px;width:100px; text-align:center;"> ' . $le_end_month . '</div>';
        $html .= '<div style="position:absolute;top:656;left:260px;width:70px; text-align:center;"> ' . $le_end_year . '</div>';

        //หน้าที่2
        $html2 = '<div style="position:absolute; top:20px; left:40px; "> เลขที่อ้างอิงสัญญาเช่า : ' . $le_no . '</div>';

        $html2 .= '<div style="position:absolute;top:113;left:520px;width:150px; text-align:center;"> ' . $dp_cost . '</div>';
        $html2 .= '<div style="position:absolute;top:140;left:120px;width:240px; text-align:center;">' . $th_dp_cost . '</div>';
        $html2 .= '<div style="position:absolute;top:166;left:115px;width:70px; text-align:center;"> ' . $dp_no . '</div>';
        $html2 .= '<div style="position:absolute;top:166;left:225px;width:55px; text-align:center;"> ' . $dp_volume . '</div>';
        $html2 .= '<div style="position:absolute;top:166;left:320px;width:32px; text-align:center;"> ' . $dp_date_pay . '</div>';
        $html2 .= '<div style="position:absolute;top:166;left:395px;width:95px; text-align:center;"> ' . $dp_month_pay . '</div>';
        $html2 .= '<div style="position:absolute;top:166;left:520px;width:50px; text-align:center;"> ' . $dp_year_pay . '</div>';

        //หน้าที่5
        $html3 = '<div style="position:absolute; top:20px; left:40px; "> เลขที่อ้างอิงสัญญาเช่า : ' . $le_no . '</div>';

        $html3 .= '<div style="position:absolute;top:473;left:90px;width:240px; text-align:center;">' . $tn_name . '</div>';
        $html3 .= '<div style="position:absolute;top:500;left:90px;width:240px; text-align:center;">' . $tn_role . '</div>';
        $html3 .= '<div style="position:absolute;top:475;left:385px;width:365px; text-align:center;">' . $boss_name . '</div>';
        $html3 .= '<div style="position:absolute;top:500;left:385px;width:365px; text-align:center;">' . $boss_role . ' ปฏิบัติหน้าที่แทน</div>';
        $html3 .= '<div style="position:absolute;top:683;left:80px;width:250px; text-align:center;">' . $wn_1 . '</div>';
        $html3 .= '<div style="position:absolute;top:710;left:80px;width:260px; text-align:center;">' . $wn_1_role . '</div>';
        $html3 .= '<div style="position:absolute;top:683;left:430px;width:305px; text-align:center;">' . $wn_2 . '</div>';
        $html3 .= '<div style="position:absolute;top:710;left:430px;width:305px; text-align:center;">' . $wn_2_role . '</div>';

        $html3 .= '<div style="position:absolute;top:525;left:390px;width:340px; text-align:center;"> อธิการบดีมหาวิทยาลัยเกษตรศาสตร์ </div>';
        
        //หน้าที่ 3-4
        $html5 = '<div style="position:absolute; top:20px; left:40px; "> เลขที่อ้างอิงสัญญาเช่า : ' . $le_no . '</div>';

        $html6 = '<div style="position:absolute; top:20px; left:40px; "> เลขที่อ้างอิงสัญญาเช่า : ' . $le_no . '</div>';
    }


    // เรียกไฟล์ สัญญาเช่า ที่จะใส่ข้อมูล จาก pdf_file_lease/lease_2_1.pdf
    //นับหน้า
    $pagecount = $mpdf->SetSourceFile('pdf_file_lease/lease_5_1.pdf'); //จาก PDF ไฟล์ไหน 
    for ($i = 1; $i <= ($pagecount); $i++) {
        $mpdf->AddPage();
        $import_page = $mpdf->ImportPage($i);
        $mpdf->UseTemplate($import_page);
...
//ถ้าเป็นจ่ายแบบใบสำคัญ
    ...
    //Set ตำแหน่งข้อมูล
    {
        ...
        $html .= '<div style="position:absolute;top:208;left:145px;width:115px; text-align:center;"> ' . $month_today . ' </div>';
        $html .= '<div style="position:absolute;top:208;left:280px;width:80px; text-align:center;"> ' . $year_today . ' </div>';
        $html .= '<div style="position:absolute;top:241;left:140px;"> ' . $boss_name . ' </div>';
        $html .= '<div style="position:absolute;top:273;left:165px;"> ' . $boss_role . ' </div>';
        $html .= '<div style="position:absolute;top:305;left:365px;"> ' . $tn_name . ' </div>';
        $html .= '<div style="position:absolute;top:337;left:135px;width:65px; text-align:center;"> ' . $tn_age . ' </div>';
        $html .= '<div style="position:absolute;top:337;left:260px;width:80px; text-align:center;"> ' . $tn_ethnicity . ' </div>';
        $html .= '<div style="position:absolute;top:337;left:385px;width:75px; text-align:center;"> ' . $tn_nationality . ' </div>';
        $html .= '<div style="position:absolute;top:337;left:540px;width:65px; text-align:center;"> ' . $tn_house_no . ' </div>';
        ...
        $html .= '<div style="position:absolute;top:369;left:450px;width:90px; text-align:center;"> ' . $tn_canton . ' </div>';
        $html .= '<div style="position:absolute;top:369;left:610px;width:100px; text-align:center;">' . $tn_district . ' </div>';
        $html .= '<div style="position:absolute;top:401;left:150px;width:100px; text-align:center;"> ' . $tn_province . ' </div>';

        $html .= '<div style="position:absolute;top:498;left:405px;">' . $area_name . ' </div>';
        $html .= '<div style="position:absolute;top:603;left:220px;width:90px; text-align:center;"> ' . $area_size . '</div>';
        $html .= '<div style="position:absolute;top:603;left:380px;">' . $le_purpose . '</div>';
        $html .= '<div style="position:absolute;top:629;left:180px;width:60px; text-align:center;">  ' . $le_duration . '</div>';
        $html .= '<div style="position:absolute;top:629;left:320px;width:45px; text-align:center;"> ' . $le_start_date . '</div>';
        $html .= '<div style="position:absolute;top:629;left:400px;width:110px; text-align:center;">' . $le_start_month . '</div>';
        $html .= '<div style="position:absolute;top:629;left:550px;width:70px; text-align:center;"> ' . $le_start_year . '</div>';
        $html .= '<div style="position:absolute;top:629;left:660px;width:40px; text-align:center;"> ' . $le_end_date . '</div>';
        $html .= '<div style="position:absolute;top:655;left:140px;width:100px; text-align:center;"> ' . $le_end_month . '</div>';
        $html .= '<div style="position:absolute;top:655;left:260px;width:70px; text-align:center;"> ' . $le_end_year . '</div>';

        //หน้าที่2
        $html2 = '<div style="position:absolute; top:20px; left:40px; "> เลขที่อ้างอิงสัญญาเช่า : ' . $le_no . '</div>';

        $html2 .= '<div style="position:absolute;top:113;left:520px;width:150px; text-align:center;"> ' . $dp_cost . '</div>';
        $html2 .= '<div style="position:absolute;top:140;left:120px;width:245px; text-align:center;"> ' . $th_dp_cost . '</div>';
        $html2 .= '<div style="position:absolute;top:166;left:115px;width:140px; text-align:center;"> ' . $dp_im  . '</div>';

        $html2 .= '<div style="position:absolute;top:166;left:307px;width:35px; text-align:center;"> ' . $dp_date_pay . '</div>';
        $html2 .= '<div style="position:absolute;top:166;left:380px;width:95px; text-align:center;"> ' . $dp_month_pay . '</div>';
        $html2 .= '<div style="position:absolute;top:166;left:505px;width:55px; text-align:center;"> ' . $dp_year_pay . '</div>';
    ...
    //นับหน้า
    $pagecount = $mpdf->SetSourceFile('pdf_file_lease/lease_5_2.pdf'); //จากไฟล์ PDF ไหน
    for ($i = 1; $i <= ($pagecount); $i++) {
        $mpdf->AddPage();
        $import_page = $mpdf->ImportPage($i);
        $mpdf->UseTemplate($import_page);
