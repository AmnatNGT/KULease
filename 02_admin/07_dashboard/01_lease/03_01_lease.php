<?php {
    ...
    // วิเคราะห์รายรับ { รายวัน } ประเภทสัญญาเพื่อร้านค้าหรือพาณิชย์

    // รับข้อมูล
    if (isset($_POST['m1'])) {
        // ค้นหา ตั้งแต่ วัน/เดือน/ปี ที่
        $m1 = $_POST['m1'];
        // ถึง วัน/เดือน/ปี ที่
        $m2 = $_POST['m2'];
    } else {
        $m1 = null;
        $m2 = null;
    ...
    //ดึงข้อมูล
    // mn_cost บวกกัน
    // วันที่จ่าย set เป็น วัน เดือน กับปี
    // นับจำนวนสัญญา
    //  ที่ >= ว/ด/ป ที่ a และ ที่ <= ว/ด/ป ที่ b
    $sql = "SELECT m.mn_cost, 
                   SUM(m.mn_cost) AS total, 
                   Date_FORMAT(m.mn_date_pay, '%d-%M-%Y') AS datesave,
                   l.le_id, count(l.le_id) AS c_le
                   FROM money m, lease l 
                   WHERE l.le_type = 1 
                   AND l.le_status!=3 AND l.le_id = m.le_id 
                   AND m.mn_status = 1 
                   AND (m.mn_type = 3 or m.mn_type = 2)
                   AND Date_FORMAT(m.mn_date_pay, '%Y-%m-%d') >= '$m1'  
                   AND Date_FORMAT(m.mn_date_pay, '%Y-%m-%d') <= '$m2' 
                   GROUP BY Date_FORMAT(m.mn_date_pay, '%Y-%m-%d') 
            ...
            <div class="form">
                ...
                <form action="" method="POST">
                    <div class="inputfield" style="width: 1000px;">
                        ...
                        <a>ค้นหา </a>
                        <!-- วัน/เดือน/ปี ที่ a -->
                        <a> >>ตั้งแต่ วัน/เดือน/ปี ที่ >></a>
                        <input type="date" value="ค้นหา" name="m1" required>

                        <!-- ถึง วัน/เดือน/ปี ที่ b -->
                        <a>>> ถึง วัน/เดือน/ปี ที่ >></a>
                        <input type="date" value="ค้นหา" name="m2" required>
                ...
                <?php if ($m1 != null) { ?>
                    <strong style="color: red;"><label for="">ข้อมูลตั้งแต่ [ <?php echo date('d/F / Y ', strtotime($m1)); ?> ] ถึง [ <?php echo date('d/F / Y ', strtotime($m2)); ?> ] </label> </strong>
                <?php } ?>
                ...
                <div class="title">
                    ตารางข้อมูลสัญญาเช่า { รายวัน } ประเภทสัญญาเพื่อร้านค้าหรือพาณิชย์
                </div>
                        ...
                        <tbody>
                            ...
                            <?php while ($row2 = mysqli_fetch_array($result2)) { ?>
                                <tr>
                                    ...
                                    <!-- ว/ด/ป -->
                                    <td><?php echo date('d / F / Y ', strtotime($row2['datesave'])); ?></td>
                                    <!-- จำนวนสัญญาเช่า -->
                            ...
                            <?php
                                // รวมเงิน
                                @$amount_total = $amount_total + $row2['total'];
                            }
                            ?>
                            <!-- รวมเงิน -->
