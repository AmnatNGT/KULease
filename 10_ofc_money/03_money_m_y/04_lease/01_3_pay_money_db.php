...
$mn_date_pay = date("Y-m-d", strtotime("+1 month", strtotime("$date_pay")));
    ...
    if ($fileError === 0) {
        ...
        //เพิ่มบิลใหม่
        {
            ...
            if ($le_duration == '1' and $mn_count < 11) {
                $sql_money = "INSERT INTO money (le_id, mn_type, mn_cost, mn_status, mn_date_pay)
                        VALUE ('$le_id','$mn_type', '$mn_cost', '$mn_status', '$mn_date_pay')";
                $result_money = mysqli_query($conn, $sql_money);
            }

            //เพิ่มบิลใหม่ 2 ปี
            if ($le_duration == '2' and $mn_count < 23) {
                $sql_money = "INSERT INTO money (le_id, mn_type, mn_cost, mn_status, mn_date_pay)
                            VALUE ('$le_id','$mn_type', '$mn_cost', '$mn_status', '$mn_date_pay')";
                $result_money = mysqli_query($conn, $sql_money);
            }
            ...
            //เพิ่มบิลใหม่ 3 ปี
            if ($le_duration == '3' and $mn_count < 35) {
                $sql_money = "INSERT INTO money (le_id, mn_type, mn_cost, mn_status, mn_date_pay)
                            VALUE ('$le_id','$mn_type', '$mn_cost', '$mn_status', '$mn_date_pay')";
                $result_money = mysqli_query($conn, $sql_money);
            }
